...
HAVE=$(sh ./configure -V | sed -ne '/generated by GNU Autoconf/s/.* \+//p')
if [[ $WANT != "$HAVE" ]]; then
  echo -ne '\e[31mWrong Autoconf version: \e[0m '
  echo -e 'Have: \e[31m'"$HAVE"'\e[0m  Want: \e[32m'"$WANT"'\e[0m'
  echo "Run tools/autogen autoconf[-]$WANT"
  exit 1
fi
...
PATHS=(tools/ci/actions/check-configure.sh configure configure.ac VERSION \
       aclocal.m4 build-aux tools/autogen tools/git-dev-options.sh)
