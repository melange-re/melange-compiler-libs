...
and check_value_name name loc =
  (* Note: we could also check here general validity of the
     ...
     -ppx preprocessors. *)
  (* TODO(EduardoRFS): seems like a weird place to hack something *)
...
let save_signature_with_transform ?check_exists cmi_transform ~alerts sg modname filename =
  ...
  Persistent_env.save_cmi ?check_exists !persistent_env
    { Persistent_env.Persistent_signature.filename; cmi } pm;
  cmi

let save_signature ?check_exists ~alerts sg modname filename =
  save_signature_with_transform ?check_exists (fun cmi -> cmi)
    ~alerts sg modname filename

let save_signature_with_imports ?check_exists ~alerts sg modname filename imports =
  let with_imports cmi = { cmi with cmi_crcs = imports } in
  save_signature_with_transform with_imports ?check_exists
    ~alerts sg modname filename
