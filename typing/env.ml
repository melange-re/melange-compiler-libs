...
let is_local_ext cda =
  match cda.cda_description with
  | {cstr_tag = Cstr_extension{path=p; _}} -> begin
      match p with
      | Pident _ -> true
...
let check_value_name name loc =
  (* Note: we could also check here general validity of the
     ...
     -ppx preprocessors. *)
  if !Config.bs_only && name = "|." then
    Error.log_and_raise (Illegal_value_name (loc, name))
  else if String.length name > 0 && not
...
let save_signature_with_transform ?check_exists cmi_transform ~alerts sg cmi_info =
  ...
  Persistent_env.save_cmi ?check_exists !persistent_env pers_sig pm;
  cmi

let save_signature ?check_exists ~alerts sg cmi =
  save_signature_with_transform ?check_exists (fun cmi -> cmi) ~alerts sg cmi

let save_signature_with_imports ?check_exists ~alerts sg cmi imports =
  let with_imports cmi = { cmi with cmi_crcs = imports } in
  save_signature_with_transform ?check_exists with_imports ~alerts sg cmi
