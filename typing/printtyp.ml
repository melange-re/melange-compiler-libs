...
let super_unification_error env tr txt1 ppf txt2 ty_expect_explanation = begin
  reset ();
  let tr = prepare_trace (fun t t' -> t, hide_variant_name t') tr in
  let mis = mismatch txt1 env tr in
  match tr with
...
let super_report_unification_error ppf env ?unif:(_unif=true)
    ?(type_expected_explanation = fun _ -> ())
    tr txt1 txt2
=
  wrap_printing_env env (fun () -> super_unification_error env tr txt1 ppf txt2 type_expected_explanation)
    ~error:true
;;
#end
