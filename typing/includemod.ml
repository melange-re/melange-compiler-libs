...
module Core_inclusion = struct
  ...
  let value_descriptions ~loc env ~direction subst id vd1 vd2 =
    ...
    try
      Ok (Includecore.value_descriptions ~loc env id vd1 vd2)
...
let core_consistency =
  ...
  in
  let value_descriptions ~loc:_ env ~direction:_ _ id vd1 vd2 =
    match Includecore.value_descriptions_consistency env vd1 vd2 id with
    | x -> Ok x
