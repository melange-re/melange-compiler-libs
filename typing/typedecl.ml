...
let transl_labels env univars closed lbls =
  ...
  if !Config.bs_only then
    ...
    | None -> ()
    | Some {loc;txt=name} -> Error.log_and_raise loc (Duplicate_label name)
  else (
    ...
    let v =
      ...
      let native_repr_args, native_repr_res =
        if !Config.bs_only then
          ...
          in
          match scann primdesc.pprim_attributes with
          | None -> parse_native_repr_attributes env pprim_type ty ~global_repr
      ...
      && not
           (List.exists
              (fun attr -> attr.attr_name.txt = "mel.internal.ffi")
              prim.prim_attrs)
      && (prim.prim_name = ""
