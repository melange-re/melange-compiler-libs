...
let rec is_nonexpansive exp =
  List.exists (function (({ attr_name = {txt = "internal.expansive"}; _}) : Parsetree.attribute) -> true | _ -> false)
   exp.exp_attributes ||
  match exp.exp_desc with
  | Texp_ident _
  | Texp_constant _
  ...
  | Texp_array (_, []) -> !Config.unsafe_empty_array
...
let id_of_pattern : Typedtree.pattern -> Ident.t option = fun pat ->
  ...
  | Tpat_construct (_,_,
                    [{pat_desc = (Tpat_var (id,_,_) | Tpat_alias(_,id,_,_,_))}], _)
...
and type_expect_
  ...
  | Pexp_ident lid ->
      ...
      rue {
        ...
        exp_env = env }
  | Pexp_constant({pconst_desc = Pconst_string (str, _, delim); _} as cst) -> (
