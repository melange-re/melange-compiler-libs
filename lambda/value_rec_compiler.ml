...
(* The compiler allows recursive definitions of functions that are not
   ...
   If the context contains let-bindings for pure expressions, we can
   also inline these expressions into the function body instead of
   storing them in the closure. See {!should_inline} below.
*)
...
(* Inlining pure definitions is useful to optimize the following
   patterns, which is generated by some ppx extensions to avoid
   hygiene issues (#14876):
   {[
     let rec hash =
       let __0 = hash in
       let __1 = Hashtbl.hash in
       function ...
   ]}
   We specifically cater for the two case above (simple variables and
   module projections) in our detection of inlinable definitions.
*)
let rec should_inline = function
  | Lvar _ -> true
  | Lprim (Pgetglobal _, [], _loc) -> true
  | Lprim (Pfield (_, _, Immutable, _), [lam], _loc) -> should_inline lam
  | _ -> false
...
let rec split_static_function env lam :
  ...
  | Lvar v ->
    ...
    let ap_func =
      Lprim (Pfield (0, Pointer, lifted_block_mut, fld_na),
    ...
    Reachable (lifted,
               Lprim (Pmakeblock (0, default_tag_info, lifted_block_mut, None),
                      [var_code], no_loc))
  | Lfunction lfun ->
    ...
    let free_vars_block_size, subst, block_fields_rev =
      Ident.Set.fold (fun var (i, subst, fields) ->
          let access =
            Lprim (Pfield (i, Pointer, lifted_block_mut, fld_na),
                   [Lvar env.block_var],
                   no_loc)
          in
  ...
  | Llet (lkind, vkind, var, def, body) ->
    let env' =
      if should_inline def then
        (* Simple renaming: use direct substitution *)
        (* We need the definition side to not mention any inlined
           variable, so we apply previous inlinings eagerly. *)
        let def_inlined = Lambda.subst (fun _ _ env -> env) env.inlined def in
        let inlined = Ident.Map.add var def_inlined env.inlined in
        { env with inlined }
      else
...
let compile_letrec = ref compile_letrec
