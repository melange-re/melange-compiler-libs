...
let can_group discr pat =
  ...
  | Constant (Const_nativeint _), Constant (Const_nativeint _) ->
      true
  | Construct { cstr_tag = Cstr_extension {path=p1; _} },
    Construct { cstr_tag = Cstr_extension {path=p2; _} }
    ->
  ...
  let make_field_accesses ~fld_info binding_kind first_pos last_pos argl =
    let rec make_args pos =
      if pos > last_pos then
        ...
        {
          arg = Lprim (Pfield (pos, Pointer, Immutable, fld_info), [ arg ], loc);
  ...
  if cstr.cstr_inlined <> None then
    ...
    | Cstr_block _ when
      ...
      ->
        begin
          let from_option =
            match head.pat_desc, get_desc head.pat_type with
          ...
          { arg = Lprim (from_option, [arg], Scoped_location.of_location ~scopes head.pat_loc)
          ; binding_kind = Alias
          ; (* TODO(anmonteiro): check this one *)
          mut = compose_mut mut Immutable;
          } :: rem
        end
    | Cstr_constant _
    | Cstr_block _ ->
        make_field_accesses Alias 0 (cstr.cstr_arity - 1) rem ~fld_info:(if cstr.cstr_name = "::" then Fld_cons else Fld_variant)
    | Cstr_unboxed -> { arg; binding_kind = Alias; mut } :: rem
    | Cstr_extension _ -> make_field_accesses Alias 1 cstr.cstr_arity rem ~fld_info:Fld_extension
...
let get_expr_args_variant_nonconst ~scopes head { arg; mut; _ } rem =
  let loc = head_loc ~scopes head in
  {
    arg = Lprim (Pfield (1, Pointer, Immutable, Fld_poly_var_content), [ arg ], loc);
    binding_kind = Alias;
    mut = compose_mut mut Immutable;
  } :: rem
...
let code_force_lazy =
  lazy (transl_prim "CamlinternalLazy" "force")
...
let inline_lazy_force_cond arg loc =
  if !Config.bs_only then
    Lapply {ap_tailcall=Default_tailcall; ap_func = Lazy.force code_force_lazy; ap_inlined = Default_inline; ap_specialised = Default_specialise; ap_args = [arg]; ap_loc = loc}
  else
  let idarg = Ident.create_local "lzarg" in
  let varg = Lvar idarg in
  let tag = Ident.create_local "tag" in
  let test_tag t =
    Lprim(Pintcomp Ceq, [Lvar tag; Lconst(Const_int (t, default_pointer_info))], loc)
  in
  Llet
    ( Strict,
      Pgenval,
        ...
        ( Alias,
          ...
          Lifthenelse
            ( (* if (tag == Obj.forward_tag) then varg.(0) else ... *)
              ...
              Lprim (Pfield (0, Pointer, Mutable, fld_na), [ varg ], loc),
              Lifthenelse
                (
          ...
          Lswitch
            ( Lprim (Pccall prim_obj_tag, [ varg ], loc),
              { sw_numblocks = 0;
                ...
                sw_consts =
                  [ (Obj.forward_tag, Lprim (Pfield(0, Pointer, Mutable, fld_na),
                                             [ varg ], loc));
  ...
  if !Clflags.afl_instrument then
    ...
    Lapply
      { ap_tailcall = Default_tailcall;
        ...
        ap_args = [ Lconst (Const_int (0, default_pointer_info)); arg ];
...
let get_expr_args_tuple ~scopes head { arg; mut; _ } rem =
  ...
  let rec make_args pos =
    if pos >= arity then
      ...
      {
        arg = Lprim (Pfield (pos, Pointer, Immutable, Fld_tuple), [ arg ], loc);
        binding_kind = Alias;
        mut = compose_mut mut Immutable;
      } :: make_args (pos + 1)
  in
  ...
  let rec make_args pos =
    if pos >= len then
      ...
      let arg =
        Lprim
          (Parrayrefu kind,
           [ arg; Lconst (Const_int (pos, default_pointer_info)) ], loc)
      in
      {
        arg;
        binding_kind =
...
let zero_lam = Lconst (Const_int (0, default_pointer_info))
...
module SArg = struct
  ...
  let make_const i = Lconst (Const_int (i, default_pointer_info))
  ...
  let make_is_nonzero arg =
    if !Clflags.native_code then
      Lprim (Pintcomp Cne,
             [arg; Lconst (Const_int (0, default_pointer_info))],
             Loc_unknown)
    else
      arg
    ...
    | Const_int _ ->
        let int_lambda_list =
          List.map
            ...
            const_lambda_list
        in
        call_switcher loc fail arg int_lambda_list names
    | Const_char _ ->
        let int_lambda_list =
          List.map
            (function
            ...
            const_lambda_list
        in
        call_switcher loc fail arg ~low:0 ~high:255 int_lambda_list names
    | Const_string _ ->
...
let combine_extension_constructor sw_names loc arg pat_env partial ctx def
      ...
      | _ ->
          ...
          if !Config.bs_only then
            ...
            Llet(Alias, Pgenval, tag,
                 Lprim(Pfield (0, Pointer, Immutable, Lambda.fld_na), [arg], loc), tests)
    in
    List.fold_right
      (fun (path, act) rem ->
        let ext = transl_extension_path loc pat_env path in
...
let combine_regular_constructor sw_names loc arg pat_env cstr partial ctx def
    ...
    | _ -> (
        ...
        | n, 0, _, [] when false (* relies on tag being an int *) -> (* The type defines constant constructors only *)
            (* The matched type defines constant constructors only.
               ...
               instruction.) *)
            call_switcher loc fail_opt arg ~low:0 ~high:(n - 1) consts sw_names
        | n, _, _, _ -> (
            let act0 =
                ...
                Lifthenelse
                  ( Lprim (Pisint, [ arg ], loc),
                    call_switcher loc fail_opt arg
                      ~low:0 ~high:(n - 1) consts sw_names,
                    act )
            | Some _ | None ->
...
let combine_constructor sw_names loc arg pat_env cstr partial ctx def actions =
  match cstr.cstr_tag with
  | Cstr_extension _ ->
    combine_extension_constructor sw_names loc arg pat_env partial ctx def actions
  | _ ->
    combine_regular_constructor sw_names loc arg pat_env cstr partial ctx def actions

let make_test_sequence_variant_constant fail arg int_lambda_list =
  let _, (cases, actions) = as_interval fail (List.map (fun (a,(_,c)) -> (a,c)) int_lambda_list) in
  Switcher.test_sequence arg cases actions

let call_switcher_variant_constant loc fail arg int_lambda_list names =
  call_switcher loc fail arg (List.map (fun (a,(_,c)) -> (a,c)) int_lambda_list) names
...
let call_switcher_variant_constr loc fail arg int_lambda_list names =
  ...
  Llet
    ( Alias,
      ...
      call_switcher loc fail (Lvar v) (List.map (fun (a,(_,c)) -> (a,c)) int_lambda_list) names)
...
let combine_variant names loc row arg partial ctx def (tag_lambda_list, total1, _pats)
  ...
  let test_int_or_block arg if_int if_block =
    Lifthenelse (Lprim (Lazy.force is_poly_var_constant, [ arg ], loc), if_int, if_block)
  in
  let sig_complete = List.length tag_lambda_list = !num_constr
  and one_action = same_actions tag_lambda_list in (* redundant work under bs context *)
...
let combine_array names loc arg kind partial ctx def (len_lambda_list, total1, _pats)
    =
  let fail, local_jumps = mk_failaction_neg partial ctx def in
  let lambda1 =
    let newvar = Ident.create_local "len" in
    let switch =
      call_switcher loc fail (Lvar newvar) ~low:0 len_lambda_list names
    in
...
and do_compile_matching ~scopes repr partial ctx pmh =
  ...
  | Pm pm -> (
      ...
      | Constant cst ->
          compile_test
            divide_constant
            (combine_constant None ploc arg cst arg_partial)
      | Construct cstr ->
          let sw_names = !names_from_construct_pattern ph
          in
          compile_test
            (divide_constructor ~scopes)
            (combine_constructor sw_names ploc arg ph.pat_env cstr arg_partial)
      | Array _ ->
          let kind = Typeopt.array_pattern_kind pomega in
          compile_test
            (divide_array ~scopes kind)
            (combine_array None ploc arg kind arg_partial)
      | Lazy ->
          compile_no_test
      ...
      | Variant { cstr_row = row } ->
          compile_test
            ...
            (combine_variant None ploc !row arg arg_partial)
    )
  | PmVar { inside = pmh } ->
      let lam, total =
...
let do_for_multiple_match ~scopes loc idl pat_act_list partial =
  ...
  let arg =
    ...
    Lprim (Pmakeblock (0, Blk_tuple, Immutable, None), args, sloc) in
...
let for_optional_arg_default ~scopes loc pat ~default_arg ~param body =
  let supplied_or_default =
    ...
    transl_match_on_option
      (Lprim(is_not_none_bs_primitve , [Lvar param], Scoped_location.of_location ~scopes loc))
      Loc_unknown
      ~if_none:default_arg
      ~if_some:
      (let from_option =
        match Typeopt.cannot_inhabit_none_like_value pat.pat_type pat.pat_env with
        | true -> val_from_unnest_option_bs_primitive
        | false -> val_from_option_bs_primitive
        in
        (Lprim
          (from_option
          , [Lvar param]
          , Scoped_location.of_location ~scopes pat.pat_loc)))
    else
    transl_match_on_option
      (Lvar param)
      Loc_unknown
      ...
      ~if_some:
        (Lprim
           (Pfield (0, Pointer, Immutable, Fld_variant),
            [ Lvar param ],
            Loc_unknown))
  in
